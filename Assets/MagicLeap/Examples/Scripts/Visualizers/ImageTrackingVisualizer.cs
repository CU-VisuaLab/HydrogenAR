...
namespace MagicLeap
{
    ...
    public class ImageTrackingVisualizer : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (_statusLabel != null) _statusLabel.text = String.Format("{0}[{1}/{2}] {3}", _prefix, _trackerBehavior.IsTracking, _trackerBehavior.TrackingStatus, _eventString);
            if (FindObjectOfType<VisSpatialMappingTracking>() != null) _demo.transform.position = FindObjectOfType<VisSpatialMappingTracking>().anchorPosition(transform.GetSiblingIndex());
        }
        ...
        private void RefreshViewMode()
        {
            switch (_lastViewMode)
            {
                case ImageTrackingExample.ViewMode.All:
                    ...
                    _demo.SetActive(alwaysOn || _targetFound);
                    break;
