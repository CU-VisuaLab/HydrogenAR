...
namespace MagicLeap
{
    ...
    public class ImageTrackingExample : MonoBehaviour
    {
        ...
        {
            /*
             * COMMENTING OUT: Not useful and I already have trigger functionality for something else.           
            if (_hasStarted && MLImageTracker.IsStarted && _controllerConnectionHandler.IsControllerValid(controllerId))
            {
                if (MLImageTracker.GetTrackerStatus())
                {
                    ...
                    if (_trackerStatusLabel != null) _trackerStatusLabel.text = "Tracker Status: Disabled";
                }
                else
                {
                    MLImageTracker.Enable();
                    if (_trackerStatusLabel != null) _trackerStatusLabel.text = "Tracker Status: Enabled";
                }
            }*/
        }
        ...
        private void HandleOnButtonDown(byte controllerId, MLInputControllerButton button)
        {
            if (_controllerConnectionHandler.IsControllerValid(controllerId) && button == MLInputControllerButton.Bumper)
            {
                ...
                if (_viewModeLabel != null) _viewModeLabel.text = string.Format("View Mode: {0}", _viewMode.ToString());
            }
            UpdateVisualizers();
        }
        #endregion
